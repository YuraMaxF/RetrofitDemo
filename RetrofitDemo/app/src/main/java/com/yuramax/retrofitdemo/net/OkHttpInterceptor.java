...
public class OkHttpInterceptor implements Interceptor {
    ...
    private static final long CACHE_CONTROL_CACHE = 60 * 60 * 24 * 1;//设缓存有效期为1天

    @Override
    public Response intercept(Chain chain) throws IOException {
        Request request = chain.request();//从chain对象中可以获取到request和response，想要的数据都可以从这里获取
        if (!NetworkUtils.isConnected()) {//如果不能上网，那么启用缓存
            request.newBuilder().cacheControl(CacheControl.FORCE_CACHE).build();
        }

        request = addHeaders(request);//增加头部信息
        logForRequest(request);//打印request日志
        Response response = chain.proceed(request);//获取response对象
        logForResponse(response);//打印response日志
        if (NetworkUtils.isConnected()) { //有网的时候读接口上的@Headers里的配置，可以在这里进行统一的设置
